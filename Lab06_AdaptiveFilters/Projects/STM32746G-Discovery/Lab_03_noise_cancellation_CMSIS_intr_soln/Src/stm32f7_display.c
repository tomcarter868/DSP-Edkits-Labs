 ...
 uint8_t CheckForUserInput(void)
{
  if (BSP_PB_GetState(BUTTON_KEY) != RESET)
  {
    ...
    if (BSP_PB_GetState(BUTTON_KEY) != RESET)  // Verify button is still pressed
    {
      while (BSP_PB_GetState(BUTTON_KEY) != RESET)  // Wait for button release
      {
        HAL_Delay(10);
      }
      HAL_Delay(20);  // Debounce on release
      return 1;
    }
  }
  return 0;
...
void plotWave(float32_t * data_buffer, int num_samples, int live, int complex) {
  ...
  //snprintf(str, sizeof(str), "%.2f", max);
	//stm32f7_LCD_init(8000.0f, str,NOGRAPH);
...
int checkButtonFlag() {
	...
	//If button flag reaches 2, wrap back to 0 to cycle between modes
	if(button_flag >= 2) {
		button_flag = 0;	
	}
